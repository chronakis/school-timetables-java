...
public class PdfGenerator {
	...
	public void writePdf(TTSheet sheet, File destFile) throws Exception {		
		PdfFont questFont = PdfFontFactory.createFont(config.questFontName, PdfEncodings.WINANSI, true, true);
		PdfFont titleFont = PdfFontFactory.createFont(config.titleFontName, PdfEncodings.WINANSI, true, true);
		...
		Style titleStyle = new Style()
			...
			.setBackgroundColor(Color.LIGHT_GRAY, 0.3f)
			;
		...
		try (PdfWriter writer = new PdfWriter(destFile.getPath())){
			...
			for (TTQuestion quest: sheet.getQuestions()) {				
				...
				if (quest.left == null)
					leftText.setBorderBottom(new SolidBorder(0.3f)).setFontColor(Color.WHITE);
				
				Text signText = new Text(quest.getSign(" \u00D7 ", " \u00F7 ")).setWidth(24f).setTextAlignment(TextAlignment.CENTER);
				Text rightText = new Text(String.format("%" + config.numberPadding + "s", quest.getRight(config.emptyString)));
				if (quest.right == null)
					rightText.setBorderBottom(new SolidBorder(0.3f)).setFontColor(Color.WHITE);
				...
				if (quest.result == null)
					resultText.setBorderBottom(new SolidBorder(0.3f)).setFontColor(Color.WHITE);
